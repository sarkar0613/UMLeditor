...
public class CLine extends Shape {
	...
	private static final int DIAMOND_HEIGHT = 10;
	Shape Hobj ;
	Shape Tobj ;
	int port1 ;
	int port2 ;
	...
	public void paint(Graphics g) {
		...
		drawDiamond(g2d, endX, endY);
	}
	
	private void drawDiamond(Graphics2D g2d, int x, int y) {
		g2d.setColor(Color.PINK);
		g2d.setStroke(new BasicStroke(4));
		int[] diamondX = { x, x + DIAMOND_WIDTH / 2, x + DIAMOND_WIDTH, x + DIAMOND_WIDTH / 2};
		int[] diamondY = { y, y - DIAMOND_HEIGHT / 2, y , y + DIAMOND_HEIGHT / 2 };
